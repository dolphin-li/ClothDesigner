...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int) ((ptrtype) (ptr) & (ptrtype) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((ptrtype) (ptr) ^ (ptrtype) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((ptrtype) (otri).tri | (ptrtype) (otri).orient)
...
#define infected(otri)                                                        \
	(((ptrtype)(otri).tri[6] & (ptrtype)2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int) ((ptrtype) (sptr) & (ptrtype) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((ptrtype) (sptr) & ~ (ptrtype) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((ptrtype) (osub).ss | (ptrtype) (osub).ssorient)
...
#define STARTINDEX 1
  ...
  b->goodangle = (float)cos(b->minangle * PI / 180.0);
  if (b->goodangle == 1.0) {
    b->offconstant = 0.0;
  } else {
    b->offconstant = 0.475 * (float)sqrt((1.0 + b->goodangle) / (1.0 - b->goodangle));
  }
...

{
  ...
  if (printvertex == (vertex) NULL)
	...
	(t->orient + 1) % 3 + 3, (ptrtype)printvertex,
  ...
  if (printvertex == (vertex) NULL)
	...
	(t->orient + 2) % 3 + 3, (ptrtype)printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
	...
	t->orient + 3, (ptrtype)printvertex,
           printvertex[0], printvertex[1]);
...

{
  ...
  printf("subsegment x%lx with orientation %d and mark %d:\n",
	  (ptrtype)s->ss, s->ssorient, mark(*s));
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
	2 + s->ssorient, (ptrtype)printvertex,
           printvertex[0], printvertex[1]);
  sdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
	3 - s->ssorient, (ptrtype)printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
	...
	4 + s->ssorient, (ptrtype)printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
	...
	5 - s->ssorient, (ptrtype)printvertex,
           printvertex[0], printvertex[1]);
}
...

{
  ...
  alignptr = (ptrtype)(pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
	  (alignptr + (ptrtype)pool->alignbytes -
	  (alignptr % (ptrtype)pool->alignbytes));
...

{
  ...
  alignptr = (ptrtype)(pool->pathblock + 1);
...

{
  ...
  alignptr = (ptrtype)m->dummytribase;
  m->dummytri = (triangle *)
	  (alignptr + (ptrtype)m->triangles.alignbytes -
	  (alignptr % (ptrtype)m->triangles.alignbytes));
  ...
  if (b->usesegments) {
	...
	alignptr = (ptrtype)m->dummysubbase;
    m->dummysub = (subseg *)
		(alignptr + (ptrtype)m->subsegs.alignbytes -
		(alignptr % (ptrtype)m->subsegs.alignbytes));
...

{
  ...
  alignptr = (ptrtype)(getblock + 1);
  foundvertex = (char *)(alignptr + (ptrtype)m->vertices.alignbytes -
	  (alignptr % (ptrtype)m->vertices.alignbytes));
...

{
  ...
  while (totalsamplesleft > 0) {
	...
	alignptr = (ptrtype)(sampleblock + 1);
    firsttri = (char *) (alignptr +
		(ptrtype)m->triangles.alignbytes -
                         (alignptr %
						 (ptrtype)m->triangles.alignbytes));
...
void init_trianglulateio(struct triangulateio *io)
{
