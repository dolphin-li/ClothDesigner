...
public:
	...
	* Constructors
	* */
	__device__ __host__ ldp_basic_vec() { memset(ptr(), 0, N*sizeof(T)); }
	__device__ __host__ ldp_basic_vec(const T*data) { memcpy(ptr(), data, N*sizeof(T)); }
	__device__ __host__ ldp_basic_vec(const T& v)
	{
		for(size_t i=0; i<N; i++)
			(*this)[i] = static_cast<T>(v);
	}
	template<class E> 
	__device__ __host__ ldp_basic_vec(const ldp_basic_vec<E, N>& rhs)
	{
		for(size_t i=0; i<N; i++)
			(*this)[i] = static_cast<T>(rhs[i]);
	}
	template<class E1, class E2, size_t M> 
	__device__ __host__ ldp_basic_vec(const ldp_basic_vec<E1, M>& A, const ldp_basic_vec<E2, N - M>& B)
	{
	...
	* */
	...
	__device__ __host__ const T& operator() (size_t i)const { return ptr()[i]; }
	__device__ __host__ T& operator() (size_t i) { return ptr()[i]; }
	__device__ __host__ const T& operator[] (size_t i)const { return ptr()[i]; }
	__device__ __host__ T& operator[] (size_t i) { return ptr()[i]; }
	...
	* */
	template<typename E>
	__device__ __host__ typename type_promote<T, E>::type dot(const ldp_basic_vec<E, N>& rhs)const
	{
	...
	__device__ __host__ T sqrLength()const
	{
		T sum = 0;
		for(size_t i=0; i<N; i++)
			sum += (*this)[i] * (*this)[i];
	...
	__device__ __host__ ldp_basic_vec<T, N> normalize()const
	{
		return (*this) / length();
	}

	__device__ __host__ ldp_basic_vec<T, N>& normalizeLocal()
	{
		(*this) /= length();
		return *this;
	}
	...
	template<typename E>
	__device__ __host__ typename type_promote<T, E>::type sqrDist(const ldp_basic_vec<E, N>& rhs)const
	{
		typename type_promote<T,E>::type sum = 0;
		for(size_t i=0; i<N; i++)
		{
...
#define LDP_BASIC_VEC_ARITHMATIC3(OP)															\
	...
	* Logic Operators: >,<,>=,<=,==,!=
	* */
	__device__ __host__ bool operator > (const ldp_basic_vec& rhs)const
	{
		for(size_t i=0; i<N; i++)
		{
			if((*this)[i] < rhs[i])
...
#define LDP_BASIC_VEC_ARITHMATIC_FRIEND(OP)																		\
	template<typename T, size_t N>																		\
	__device__ __host__ inline ldp_basic_vec<T, N>operator OP (const T& lhs, const ldp_basic_vec<T, N>& rhs)		\
	{																											\
		ldp_basic_vec<T, N> out;																\
		for(size_t i=0; i<N; i++)																				\
			out[i] = lhs OP rhs[i];																				\
...
{
public:
	__device__ __host__ ldp_basic_vec2(){ (*this)[0] = 0, (*this)[1] = 0; }
	__device__ __host__ ldp_basic_vec2(const T& v){ (*this)[0] = v, (*this)[1] = v; }
	__device__ __host__ ldp_basic_vec2(const T& xx, const T& yy){ (*this)[0] = xx, (*this)[1] = yy; }
	template<class E> 
	__device__ __host__ ldp_basic_vec2(const ldp_basic_vec<E, 2>& rhs){ (*this)[0] = T(rhs[0]), (*this)[1] = T(rhs[1]); }

	__device__ __host__ const T& x()const{ return (*this)[0]; }
	__device__ __host__ T& x(){ return (*this)[0]; }
	__device__ __host__ const T& y()const{ return (*this)[1]; }
	__device__ __host__ T& y(){ return (*this)[1]; }

	template<class E> 
	__device__ __host__ typename type_promote<T, E>::type cross(const ldp_basic_vec<E, 2>& rhs)const
	{
...
{
public:
	__device__ __host__ ldp_basic_vec3(){ (*this)[0] = 0, (*this)[1] = 0, (*this)[2] = 0; }
	__device__ __host__ ldp_basic_vec3(const T& v){ (*this)[0] = v, (*this)[1] = v, (*this)[2] = v; }
	__device__ __host__ ldp_basic_vec3(const T& xx, const T& yy, const T& zz){ (*this)[0] = xx, (*this)[1] = yy, (*this)[2] = zz; }
	template<class E> 
	__device__ __host__ ldp_basic_vec3(const ldp_basic_vec<E, 3>& rhs){ (*this)[0] = T(rhs[0]), (*this)[1] = T(rhs[1]), (*this)[2] = T(rhs[2]); }

	__device__ __host__ const T& x()const{ return (*this)[0]; }
	__device__ __host__ T& x(){ return (*this)[0]; }
	__device__ __host__ const T& y()const{ return (*this)[1]; }
	__device__ __host__ T& y(){ return (*this)[1]; }
	__device__ __host__ const T& z()const{ return (*this)[2]; }
	__device__ __host__ T& z(){ return (*this)[2]; }

	template<class E> 
	__device__ __host__ typename ldp_basic_vec3<typename type_promote<T, E>::type> cross(const ldp_basic_vec<E, 3>& rhs)const
	{
...
{
public:
	__device__ __host__ ldp_basic_vec4(){ (*this)[0] = 0, (*this)[1] = 0, (*this)[2] = 0, (*this)[3] = 0; }
	__device__ __host__ ldp_basic_vec4(const T& v){ (*this)[0] = v, (*this)[1] = v, (*this)[2] = v, (*this)[3] = v; }
	__device__ __host__ ldp_basic_vec4(const T& xx, const T& yy, const T& zz, const T& ww){ (*this)[0] = xx, (*this)[1] = yy, (*this)[2] = zz, (*this)[3] = ww; }
	template<class E> 
	__device__ __host__ ldp_basic_vec4(const ldp_basic_vec<E, 4>& rhs)
	...
	__device__ __host__ const T& z()const{ return (*this)[2]; }
	__device__ __host__ T& z(){ return (*this)[2]; }
	__device__ __host__ const T& w()const{ return (*this)[3]; }
	__device__ __host__ T& w(){ return (*this)[3]; }
};
